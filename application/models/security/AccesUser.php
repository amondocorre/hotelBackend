...
class AccesUser extends CI_Model {
  ...
  public function findByUser($idUser) {
    ...
    (SELECT CONCAT("[", GROUP_CONCAT(\'"\', abu.id_boton, \'"\'), "]") FROM acceso_boton_usuario abu WHERE abu.id_acceso = ma.id_menu_acceso AND abu.estado = 1 AND abu.id_usuario = '.$this->db->escape($idUser).') AS botones_user';
  ...
  function getSubMenu($access,$idSubMenu,$nivel){
    ...
    foreach ($access as $key => $acces) {
      if($acces->nivel_superior === $idSubMenu){
        ...
        if(property_exists($acces, 'botones_user')){
          $acces->botones_user = $acces->botones_user?json_decode($acces->botones_user):[];
        }
