...
class Company extends CI_Model {
  ...
  public function getId($company) {
      return $mascota->id_empresa ?? null;
  }
  public function getDataId($id) {
      $url = getHttpHost();
      $this->db->select("id_empresa,nombre,direccion,celular,correo,CONCAT('$url', logo_empresa) as logo_empresa,CONCAT('$url', logo_impresion) as logo_impresion");
  ...
  public function findAll() {
    $url = getHttpHost();
    $this->db->select("id_empresa,nombre,nit,direccion,telefono,celular,correo,pagina_web,ubicacion_gps,pie_documento,created_at,updated_at,CONCAT('$url', logo_empresa) as logo_empresa,CONCAT('$url', logo_impresion) as logo_impresion");
  ...
  private function validate_pet_data($data, $id_empresa = 0) {
    ...
    $this->form_validation->set_rules('nit', 'Nit', 'max_length[15]'.($id_empresa?'':' | is_unique[empresa.nit]'));
